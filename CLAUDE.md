...
4. **Criação do jogo em si** — 🔄 em andamento. Mecânica de gestos isolada: config default ([lib/game-config.js](lib/game-config.js)), detector puro testável ([lib/gesture-detector.js](lib/gesture-detector.js)) e página de teste [pages/gesture-counter/](pages/gesture-counter/). Acerto = inclinar para baixo (gamma diminui), skip = oposto. Detalhes em [docs/fase-4-jogo.md](docs/fase-4-jogo.md).
